...
import { createJob } from "@/lib/data/jobs.client";
...
export default function NewJobPage() {
  ...
  const show = (k: string) => (submitted || touched[k]) && (issues as any)[k];
  ...
  async function createJobAndGo() {
    const number = `J-${Math.floor(1000 + Math.random() * 9000)}`;
    ...
    router.push(`/app/jobs/${job.id}/invoice`); // direct to invoice flow
  }
  ...
  return (
    <div className="pb-20">
                ...
                <form onSubmit={continueToReview} className="space-y-6">
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div className="space-y-2">
                      ...
                      <Input id="rego" required placeholder="ABC123"
                        value={rego} onChange={(e) => setRego(e.target.value)}
                    ...
                    <div className="space-y-2">
                      ...
                      <Input id="customer" required placeholder="John Smith"
                  ...
                  <div className="space-y-2">
                    ...
                    <Input id="service" placeholder="WOF + Oil Change" value={service} onChange={(e) => setService(e.target.value)} />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Textarea id="notes" rows={4} placeholder="Any special instructions…" value={notes} onChange={(e) => setNotes(e.target.value)} />
                  </div>
                </form>
              </CardContent>
            </Card>
              ...
              <CardContent>
                ...
                <div className="space-y-4 text-sm">
                  ...
                  <div>
                    ...
                    <div className="min-h-12 rounded-md border bg-card px-3 py-2">{notes ? notes : <span className="text-muted-foreground">No notes</span>}</div>
                  </div>
                </div>
                <Separator className="my-6" />
                <div className="text-xs text-muted-foreground">Tip: You can edit fields by going back to <b>Details</b>.</div>
              </CardContent>
            </Card>
          )}
        </div>
      ...
      <StickyActions
        left={step === 2 ? (<button className="text-sm text-muted-foreground underline-offset-4 hover:underline" onClick={() => setStep(1)}>← Back to Details</button>) : null}
        right={step === 1 ? (
          <>
